...
public class CalculatorServiceExtendedImpl implements CalculatorService {

    private static final List<String> AVAILABLE_OPERATIONS = Arrays.asList("+", "-", "*", "/");
    private static final List<String> HIGH_PRIORITY_OPERATIONS = Arrays.asList("*", "/");
    private static final List<String> LOWER_PRIORITY_OPERATIONS = Arrays.asList("+", "-");
        ...
        for (String elem : expression.trim().split(" ")) {
            if (isNumeric(elem)) {
                ...
                if (!operationQueue.isEmpty()) {
                    ...
                    if (HIGH_PRIORITY_OPERATIONS.contains(elem)) {
                        if (LOWER_PRIORITY_OPERATIONS.contains(prevOperation)) {
                            operationQueue.offerFirst(prevOperation);
                        } else {
                            result.add(prevOperation);
                        }
                    } else {
                        if (LOWER_PRIORITY_OPERATIONS.contains(prevOperation)) {
                            result.add(prevOperation);
